...
fun main() {
    ...
    indexes = indexes.map { (value, code) ->
        (value - last.first).absoluteValue to code
    }

    val product = indexes.map { it.second }.reduce { acc, next -> acc * next }
    val partials = indexes.map { it.second }.map { product / it }

    val inverses = indexes.map { it.second }
        .mapIndexed { i, value ->
            modInverse(partials[i], value)
        }

    val sum = inverses.mapIndexed { i, inverse ->
        partials[i] * inverse * indexes[i].first
    }.sum()
