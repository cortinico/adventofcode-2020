fun main() {
    ...
    world
        .fold(0) { acc, next ->
            acc +
                next.fold(0) { acc2, next2 ->
                    acc2 + next2.fold(0) { acc3, chars -> acc3 + chars.count { it == '#' } }
                }
        }
        .also(::println)
}

fun create4DWorld(size: Int) =
    Array(size) { Array(size) { Array(size) { CharArray(size) { '.' } } } }
...
fun countActiveNeighbors(array: Array<Array<Array<CharArray>>>, m: Int, k: Int, i: Int, j: Int) =
    listOf(
